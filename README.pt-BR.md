...
| `POST` | `/api/v1/auth/refresh` | `{refresh_token}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_SESSION` (com `WWW-Authenticate: Bearer`), `422 VALIDATION_ERROR` |
| `POST` | `/api/v1/auth/logout` | `{refresh_token}` | `204`, também para um token desconhecido, vencido ou já revogado; `422 VALIDATION_ERROR` |
...
**Bloqueio de login.** Depois de `LOGIN_MAX_FAILURES` (padrão `5`) logins que falharam para um e-mail dentro de `LOGIN_LOCKOUT_WINDOW` (padrão `15m`), todo login desse e-mail responde `429 TOO_MANY_ATTEMPTS` com `Retry-After` (os segundos que faltam na janela) até a janela acabar, mesmo com a senha certa. A contagem é por e-mail, somando todos os endereços de cliente, vale para e-mails desconhecidos exatamente como para os cadastrados, então um `429` não revela quais e-mails existem, e um login bem-sucedido a zera. O gateway repassa o `Retry-After` do account service nas respostas `429` e `503`. Qualquer um consegue bloquear o e-mail de outra pessoa por 15 minutos com cinco senhas erradas; esse é o preço de um limite que o atacante não consegue espalhar por vários endereços.
...
- [x] **Sprint 12** — sessões: refresh tokens rotativos com logout, bloqueio de login por conta e consulta de titular reforçada

...
| `JWT_TTL` | `15m` | Validade de um token de acesso (`expires_in` em segundos), no máximo `24h`: um valor maior impede o gateway de subir; um valor não positivo cai para o padrão. O cliente obtém um novo com o seu refresh token (veja [Autenticação e autorização](#autenticação-e-autorização)) |
...
| `OWNER_NEGATIVE_CACHE_TTL` | `5s` | Por quanto tempo o gateway lembra que o account service não conhece uma conta; `0` desliga e um valor negativo impede o gateway de subir |
...
O account service também guarda as identidades da plataforma, em `identities_by_email` (`migrations/008_identities_by_email.cql`: e-mail, id do usuário, hash bcrypt da senha com custo 12, data de criação), e as sessões de refresh que o gateway entrega junto com os tokens de acesso. O gateway chama estes endpoints internos no cadastro, no login, no refresh e no logout; eles não são repassados pelo gateway:
...
| `POST` | `/identities/verify` | `{email, password}` | `200` `{user_id}`, `401 INVALID_CREDENTIALS`, `429 TOO_MANY_ATTEMPTS` (com `Retry-After`), `503 SERVICE_BUSY` |
| `POST` | `/sessions` | `{user_id}` | `201` `{refresh_token, expires_at}` (uma sessão nova), `422 VALIDATION_ERROR` (`user_id`) |
| `POST` | `/sessions/rotate` | `{refresh_token}` | `200` `{user_id, refresh_token, expires_at}`, `401 INVALID_SESSION` |
| `POST` | `/sessions/revoke` | `{refresh_token}` | `204`, também para um token desconhecido, vencido ou já revogado |
...
O bcrypt é lento de propósito, então o serviço roda no máximo `IDENTITY_HASH_CONCURRENCY` hashes e comparações ao mesmo tempo (padrão: o dobro de `GOMAXPROCS`; valores abaixo de `1` voltam ao padrão), inclusive a comparação fictícia. Uma requisição que não consegue começar o hash em 2 s, ou cujo cliente desiste antes, responde `503 SERVICE_BUSY` sem mexer nas identidades gravadas, então uma rajada de logins espera um pouco na fila e depois é descartada em vez de sufocar o resto do serviço. Todo `503 SERVICE_BUSY` responde `Retry-After: 1`.

**Sessões.** Um refresh token tem 32 bytes aleatórios em base64url, e só o hash SHA-256 dele é gravado, em `refresh_tokens` (migrations 009 e 012) com um TTL de `REFRESH_TOKEN_TTL` (padrão `720h`). Todo login ou cadastro começa uma família nova; `POST /sessions/rotate` marca o token recebido como `rotated` com uma lightweight transaction e emite o próximo token da mesma família, que vence `REFRESH_TOKEN_TTL` depois, mas nunca depois de a família completar `REFRESH_FAMILY_MAX_AGE` (padrão `2160h`), então uma sessão precisa fazer login de novo pelo menos a cada 90 dias. Um refresh token funciona uma vez: apresentar um token `rotated` ou `revoked`, ou perder a corrida para rotacionar o mesmo token, revoga a família inteira e responde `401 INVALID_SESSION`. Um token roubado não é recusado por si só: quem o apresenta primeiro recebe um token novo e pode continuar rotacionando, e a família é revogada assim que qualquer um dos dois apresenta um token que já foi usado — normalmente o dono, no próximo refresh. Se o dono nunca mais fizer refresh, o ladrão mantém a sessão até a família atingir `REFRESH_FAMILY_MAX_AGE`. Um token desconhecido, malformado ou vencido responde o mesmo `401`. `POST /sessions/revoke` revoga a família do token recebido. A revogação grava um marcador em `revoked_families` (migration 011) antes de marcar cada token da família (`sessions_by_family`, migration 010) como `revoked`, e uma rotação confere o marcador antes e depois de rotacionar, então uma revogação vence uma rotação que roda ao mesmo tempo. Um `REFRESH_TOKEN_TTL` ou `REFRESH_FAMILY_MAX_AGE` que não é positivo cai para o padrão.

**Bloqueio de login.** `POST /identities/verify` conta as tentativas por e-mail normalizado em `login_failures` (migration 013), para e-mails desconhecidos exatamente como para os cadastrados, então um bloqueio não revela se um e-mail existe. Cada tentativa é reservada com uma lightweight transaction antes de a senha ser conferida, então no máximo `LOGIN_MAX_FAILURES` (padrão `5`) senhas são comparadas por e-mail por janela, por mais requisições que cheguem ao mesmo tempo. Depois de `LOGIN_MAX_FAILURES` tentativas contarem dentro de `LOGIN_LOCKOUT_WINDOW` (toda tentativa reservada conta, inclusive as que ainda estão sendo conferidas, até um sucesso zerar a contagem) (padrão `15m`, uma janela fixa que começa na primeira falha e é medida pelo relógio do serviço), toda verificação desse e-mail responde `429 TOO_MANY_ATTEMPTS` com `Retry-After` igual aos segundos que faltam na janela, mesmo com a senha certa. Um login bem-sucedido zera a contagem; uma tentativa que falha por qualquer motivo que não seja credencial errada — `503 SERVICE_BUSY`, ou um erro na busca da identidade que responde `500` — devolve a sua reserva. Uma tentativa que continua perdendo a corrida para tentativas simultâneas no mesmo e-mail, ou cuja gravação da reserva tem resultado desconhecido, responde `429 TOO_MANY_ATTEMPTS` com `Retry-After: 1`; quando a contagem não pode ser lida ou gravada, a verificação responde `503 SERVICE_BUSY` sem comparar a senha. Um e-mail que o cadastro recusaria como malformado não é contado e não gera linha: responde `401 INVALID_CREDENTIALS` depois da mesma comparação com o hash fictício feita para um e-mail desconhecido. `LOGIN_MAX_FAILURES` abaixo de `1` e um `LOGIN_LOCKOUT_WINDOW` não positivo caem para os padrões.

**Limitações conhecidas.** Quem conhece um e-mail consegue bloqueá-lo por `LOGIN_LOCKOUT_WINDOW` com `LOGIN_MAX_FAILURES` senhas erradas (cinco requisições no padrão), um preço aceito para limitar tentativas espalhadas por vários endereços. `POST /sessions` confia no `user_id` que recebe, então quem alcança a porta do serviço consegue abrir uma sessão para qualquer usuário: só o gateway pode alcançá-la, e o compose do serviço a publica só em `127.0.0.1:8082`. Uma rotação cuja gravação no Cassandra estoura o tempo pode ter sido aplicada mesmo respondendo erro: um cliente que tenta de novo com o mesmo refresh token revoga a própria família e precisa fazer login de novo.
...
Toda rota de `/api/v1`, exceto `POST /api/v1/auth/register`, `/auth/login`, `/auth/refresh`, `/auth/logout` e `GET /api/v1/openapi.yaml`, exige um token de acesso em `Authorization: Bearer <token>`. Os usuários se cadastram e entram com e-mail e senha: o gateway os confere pelos endpoints internos de identidade do account service (veja [Account Service](#account-service)), assina o token de acesso ele mesmo e abre uma sessão de refresh no account service.
...
| `POST` | `/api/v1/auth/login` | `{email, password}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_CREDENTIALS` (com `WWW-Authenticate: Bearer`), `422 VALIDATION_ERROR`, `429 TOO_MANY_ATTEMPTS` (com `Retry-After`), `503 SERVICE_BUSY` |
| `POST` | `/api/v1/auth/refresh` | `{refresh_token}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_SESSION` (com `WWW-Authenticate: Bearer`; também para um token com mais de 256 bytes, recusado sem chamar o account service), `422 VALIDATION_ERROR`, `503 SERVICE_BUSY` |
| `POST` | `/api/v1/auth/logout` | `{refresh_token}` | `204`, também para um token desconhecido, vencido ou já revogado e para um com mais de 256 bytes, que nunca chega ao account service; `422 VALIDATION_ERROR`, `503 SERVICE_BUSY` |

O e-mail tem os espaços das pontas removidos e é passado para minúsculas, então `Ana@Example.com` e `ana@example.com` são o mesmo usuário. O cadastro aceita um e-mail válido de até 254 caracteres (`details` `email`: `required`, `email` ou `max`) e uma senha de 8 a 72 bytes (`password`: `required` ou `length`), e responde com um id de usuário aleatório novo. O login só exige os dois campos (`required`) e dá o mesmo `401 INVALID_CREDENTIALS` para um e-mail desconhecido, uma senha errada, um e-mail que nunca poderia ter sido cadastrado e uma senha com mais de 72 bytes, então a resposta não diz qual foi o caso. O cadastro, por outro lado, revela se um e-mail já está cadastrado: um segundo cadastro responde `409 EMAIL_TAKEN`, e é por isso que as duas rotas ficam atrás do limite mais rígido descrito abaixo. `/auth/refresh` e `/auth/logout` respondem `422 VALIDATION_ERROR` (`details` `refresh_token`: `required`) sem um refresh token. Os quatro respondem `413 PAYLOAD_TOO_LARGE` para um corpo acima de 16 KiB, o cadastro e o login respondem `503 SERVICE_BUSY` com `Retry-After: 1` quando o account service já está calculando tantos hashes de senha quanto permite (e o login também quando o account service não consegue ler ou gravar os contadores de falhas de login), e os quatro respondem `502 UPSTREAM_UNAVAILABLE` quando o account service não responde ou responde algo inesperado, inclusive um redirect (o gateway nunca segue um).

**Tokens.** O token de acesso é um JWT assinado com HMAC-SHA256 (HS256) e o `JWT_SECRET`, com `sub` (o id do usuário), `iss` (`fintech-gateway`), `iat`, `exp` e `jti` (um id aleatório). Ele vale por `JWT_TTL` (padrão `15m`, no máximo `24h`); `expires_in` é essa validade em segundos. O gateway só aceita tokens HS256 emitidos por ele mesmo: qualquer outro `alg` (inclusive `none`), outro emissor, uma assinatura inválida, um `exp` ausente ou vencido (com 30 s de tolerância para diferença de relógio) ou um `sub` que não é um id de usuário é recusado. Não há lista de revogação para tokens de acesso: o logout encerra a sessão, mas um token de acesso já emitido continua valendo até vencer, assim como um que vazou. Um token ausente, malformado ou recusado responde `401 UNAUTHORIZED` com `WWW-Authenticate: Bearer`. O gateway remove o header `Authorization` antes de repassar uma leitura, então os serviços de domínio nunca veem o token.

**Sessões.** O cadastro e o login também devolvem um refresh token, uma string opaca que o account service guarda só como hash SHA-256, e `refresh_expires_in`, a validade dele em segundos (`REFRESH_TOKEN_TTL`, padrão `720h`). `POST /auth/refresh` troca esse token por um token de acesso novo e um refresh token novo; o enviado deixa de funcionar. Todo refresh token emitido a partir de um login ou cadastro pertence à mesma sessão: mandar um refresh token que já foi usado revoga a sessão inteira, então o token mais novo também deixa de funcionar e o cliente precisa fazer login de novo — seja quem for que mande o token usado, o dono ou alguém que roubou um token, e uma sessão não pode ser renovada depois de `REFRESH_FAMILY_MAX_AGE` (padrão `2160h`) do seu login. `POST /auth/logout` revoga a sessão do refresh token recebido e responde `204` esteja o token ainda válido ou não, então pode ser repetido com segurança; ele responde `502 UPSTREAM_UNAVAILABLE` quando o account service não responde ou falha, e nesse caso o resultado é desconhecido: a sessão pode ter sido revogada ou não, então repita o logout, que é idempotente. Um refresh que responde `5xx` pode ter rotacionado o token mesmo assim: repeti-lo com o mesmo refresh token revoga a sessão, e o cliente faz login de novo. Os detalhes estão em [Account Service](#account-service).

**Bloqueio de login.** Depois de `LOGIN_MAX_FAILURES` (padrão `5`) tentativas de login de um e-mail contarem dentro de `LOGIN_LOCKOUT_WINDOW` (padrão `15m`; toda tentativa conta até um login bem-sucedido zerar a contagem), todo login desse e-mail responde `429 TOO_MANY_ATTEMPTS` com `Retry-After` (os segundos que faltam na janela) até a janela acabar, mesmo com a senha certa. A contagem é por e-mail, somando todos os endereços de cliente, vale para e-mails desconhecidos exatamente como para os cadastrados, então um `429` não revela quais e-mails existem, e um login bem-sucedido a zera. Um e-mail malformado não é contado: responde `401 INVALID_CREDENTIALS` como um desconhecido. O gateway repassa o `Retry-After` do account service nas respostas `429` e `503` e, por padrão, o inclui em `CORS_EXPOSED_HEADERS`, para que clientes no navegador consigam lê-lo. Qualquer um consegue bloquear o e-mail de outra pessoa por 15 minutos com cinco senhas erradas; esse é o preço de um limite que o atacante não consegue espalhar por vários endereços.

**Rate limit.** Além do limite geral, `/auth/register`, `/auth/login`, `/auth/refresh` e `/auth/logout` dividem um limite mais rígido: `AUTH_RATE_LIMIT_REQUESTS` (padrão `10`) por `AUTH_RATE_LIMIT_WINDOW` (padrão `1m`), contado por endereço do cliente exatamente como o geral (um cliente IPv6 pela sua rede /64) e respondendo `429 RATE_LIMIT_EXCEEDED`. Ele atrasa quem tenta adivinhar senhas; aumente-o só para rodar testes (veja [Testes end-to-end e de carga](#testes-end-to-end-e-de-carga)). O limite é por endereço do cliente; tentativas espalhadas por muitos endereços são limitadas por e-mail pelo bloqueio de login descrito acima.
...
Qualquer outro caso responde `403 FORBIDDEN`. O titular de uma conta vem do endpoint interno `GET /accounts/{id}/owner` do account service e fica na memória do gateway por `OWNER_CACHE_TTL` (padrão `1m`, no máximo 10.000 contas); o titular de uma conta nunca muda, então o cache não entrega uma conta a outra pessoa. Consultas simultâneas da mesma conta compartilham uma única chamada ao account service. Uma conta que o account service não conhece responde `404 ACCOUNT_NOT_FOUND` pelo próprio gateway, seja ela citada no caminho ou no corpo de um comando, e essa resposta fica guardada por `OWNER_NEGATIVE_CACHE_TTL` (padrão `5s`, `0` desliga, no máximo 1.000 contas desconhecidas, guardadas à parte dos titulares para que uma enxurrada de ids aleatórios nunca tire um titular conhecido do cache), então uma rajada de requisições para um id desconhecido custa uma consulta, enquanto uma conta criada há pouco é encontrada no máximo esse tempo depois de passar a existir; uma consulta que falha responde `502 UPSTREAM_UNAVAILABLE` e não vai para o cache. O corpo de um comando é validado primeiro, então um comando inválido ainda responde `422` antes de a conta ser consultada. Um id de conta no caminho de `GET /accounts/{id}`, `GET /accounts/{account_id}/transactions` ou `GET /accounts/{account_id}/payments` que não é um UUID responde `422 VALIDATION_ERROR` (`details` `{"id":"uuid"}` ou `{"account_id":"uuid"}`) pelo próprio gateway, antes de qualquer consulta; um id com percent-encoding é decodificado para a checagem de titularidade e depois recusado do mesmo jeito, então nunca chega a um serviço. `GET /transactions/{id}` e `GET /payments/{id}` buscam o registro e só o entregam ao titular, então um id desconhecido continua respondendo `404 TRANSACTION_NOT_FOUND` ou `404 PAYMENT_NOT_FOUND`, enquanto o registro de outro usuário responde `403`. Quem recebe uma transferência consegue lê-la pelo id, mas o gateway remove antes a `description` e a `idempotency_key`, que foram escolhidas por quem enviou.

Os serviços atrás do gateway confiam nele: não há autenticação entre serviços, então as portas deles (`8082`–`8085`), com os endpoints internos de identidade, de sessão e de titular do account service, não podem ficar acessíveis de fora. Os arquivos de compose só as publicam para desenvolvimento, a do account service só em `127.0.0.1`, já que o `POST /sessions` dele abre uma sessão para qualquer id de usuário que receber; o webhook do payment service, que confere a própria assinatura, é a única rota que um provedor precisa alcançar.
...
O segundo refresh reusa um token que já foi rotacionado, então também revogou a sessão: o refresh token devolvido pelo primeiro também deixa de funcionar. `POST /api/v1/auth/logout` com `{"refresh_token": …}` encerra uma sessão de propósito e responde `204`.
...
**Atualizando de uma versão anterior à Sprint 12.** O padrão de `JWT_TTL` agora é `15m` em vez de `1h`; defina-o explicitamente para manter a validade antiga. O cadastro e o login agora também devolvem `refresh_token` e `refresh_expires_in`, que os clientes que os ignoram podem continuar ignorando, fazendo login de novo quando o token de acesso vence. Os tokens de acesso emitidos antes da atualização continuam valendo até vencer. O account service aplica as migrations 009 a 013 (`refresh_tokens`, `sessions_by_family`, `revoked_families`, `refresh_tokens.family_created_at`, `login_failures`) no boot; faça o deploy dele antes do gateway ou junto, porque o cadastro e o login chamam o novo endpoint de sessão e respondem `502 UPSTREAM_UNAVAILABLE` enquanto ele não existe. Cinco logins que falharam para um e-mail em 15 minutos agora o bloqueiam pelo resto da janela; qualquer coisa que faça login repetidamente com uma senha errada de propósito, como um teste, precisa de um e-mail novo para cada caso.
...
│   │   │   ├── app/handlers/              endpoints de comando, cadastro/login/refresh/logout, guarda de titularidade, proxy de leitura, handler do OpenAPI
│   │   │   └── infrastructure/
│   │   │       ├── auth/                  emissor e verificador de JWT, middleware bearer
│   │   │       ├── identity/              cliente de identidades e sessões do account service
│   │   │       ├── owners/                consulta do titular da conta com cache em memória e consultas simultâneas agrupadas
...
│   │   │   │   ├── services/              casos de uso de contas, clientes, identidades e sessões
│   │   │   │   └── handlers/              dispatcher de comandos, DLQ, endpoints de leitura, de identidade e de sessão
│   │   │   └── infrastructure/
...
- [x] **Sprint 12** — sessões: refresh tokens rotativos com logout, bloqueio de login por e-mail e consulta de titular reforçada

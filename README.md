...
The e-mail is trimmed and lower-cased, so `Ana@Example.com` and `ana@example.com` are the same user. Registration takes a valid e-mail of at most 254 characters (`details` `email`: `required`, `email` or `max`) and a password of 8 to 72 bytes (`password`: `required` or `length`), and answers with a new random user id. Login only requires both fields (`required`) and gives the same `401 INVALID_CREDENTIALS` for an unknown e-mail, a wrong password, an e-mail that could never have been registered and a password longer than 72 bytes, so the answer doesn't tell which one it was. Registration, on the other hand, does reveal whether an e-mail is already registered: a second registration answers `409 EMAIL_TAKEN`, which is why both routes sit behind the stricter rate limit below. `/auth/refresh` and `/auth/logout` answer `422 VALIDATION_ERROR` (`details` `refresh_token`: `required`) without a refresh token. All four answer `413 PAYLOAD_TOO_LARGE` for a body over 16 KiB, registration and login answer `503 SERVICE_BUSY` with `Retry-After: 1` when the account service is already hashing as many passwords as it allows, and all four answer `502 UPSTREAM_UNAVAILABLE` when the account service can't be reached or answers anything unexpected, a redirect included (the gateway never follows one).
...
**Sessions.** Registration and login also return a refresh token, an opaque string the account service stores only as a SHA-256 hash, and `refresh_expires_in`, its lifetime in seconds (`REFRESH_TOKEN_TTL`, default `720h`). `POST /auth/refresh` exchanges it for a new access token and a new refresh token; the one sent stops working. Every refresh token issued from one login or registration belongs to the same session: sending a refresh token that was already used revokes the whole session, so the newest token stops working too and the client has to log in again, and a session can't be refreshed past `REFRESH_FAMILY_MAX_AGE` (default `2160h`) after its login. `POST /auth/logout` revokes the session of the refresh token it gets and answers `204` whether or not the token was still valid, so it can be retried safely; it answers `502 UPSTREAM_UNAVAILABLE` when the account service can't be reached, and then the session is still alive. A refresh that answers a `5xx` may still have rotated the token: retrying it with the same refresh token then revokes the session, and the client logs in again. The details are in [Account Service](#account-service).

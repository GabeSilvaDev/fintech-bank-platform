...
| API Gateway | `services/api-gateway/` | Chi router with request-id, tracing, metrics, client IP (the connection address, or a trusted `X-Forwarded-For` hop with `TRUST_PROXY_HEADERS=true`), logging, recovery, CORS and rate-limit middleware; `GET /health`; registration, login, refresh and logout (`POST /api/v1/auth/register`, `/auth/login`, `/auth/refresh`, `/auth/logout`, under their own rate limit) issuing short-lived HS256 JWT access tokens with rotating refresh tokens, bearer authentication on every other `/api/v1` route and owner-only access checked against the account service's owner endpoint (see [Authentication and authorization](#authentication-and-authorization)); command endpoints publishing to Kafka through a circuit-breaker-guarded producer; typed config from env; OpenAPI 3.1 document served at `GET /api/v1/openapi.yaml` and linted with Redocly in CI; unit + feature tests at 100 % coverage, Kafka integration test in CI; read routes proxied to the domain services |
| Account Service | `services/account-service/` | Consumes `account.commands`, retries its Cassandra bootstrap, migrations and keyspace session at start-up (`STARTUP_RETRY_*`), persists customers and accounts in Cassandra (`fintech_accounts`), owns balances with compare-and-set credits/debits that require an `idempotency_key` and apply at most once (`balance_operations`, 30-day TTL), publishes results — including `account.credit_rejected` — on `account.events` and failures on `account.dlq`; stores e-mail/password identities (`identities_by_email`, bcrypt) with a per-e-mail login lockout (`login_failures`) and rotating refresh sessions (`refresh_tokens`, `sessions_by_family`, `revoked_families`) behind internal endpoints the gateway calls for registration, login, refresh and logout; read API on `:8082`; unit + feature tests at 100 % of `internal/app`, Cassandra and Kafka integration tests in CI |
...
Or natively: `make run` (listens on `SERVER_PORT`, default 8080). Configuration is read from the environment: `SERVER_*` (host, port, timeouts), `CORS_*` (`CORS_ALLOWED_METHODS` defaults to `GET,POST,PUT,PATCH,DELETE,OPTIONS`, `CORS_ALLOW_CREDENTIALS` to `false` and `CORS_EXPOSED_HEADERS` to `Link,X-Next-Before,Retry-After`), `RATE_LIMIT_REQUESTS` / `RATE_LIMIT_WINDOW`, `KAFKA_BROKERS` / `KAFKA_WRITE_TIMEOUT` / `KAFKA_BATCH_TIMEOUT` / `KAFKA_PUBLISH_TIMEOUT` / `KAFKA_MAX_ATTEMPTS` / `KAFKA_BREAKER_*`, `TRUST_PROXY_HEADERS` / `TRUSTED_PROXY_HOPS`, `JWT_SECRET` / `JWT_TTL`, `AUTH_RATE_LIMIT_REQUESTS` / `AUTH_RATE_LIMIT_WINDOW`, `OWNER_CACHE_TTL` / `OWNER_NEGATIVE_CACHE_TTL`, `METRICS_ENABLED` / `OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_SAMPLER_RATIO` and `LOG_LEVEL` / `LOG_PRETTY`.
...
| `JWT_TTL` | `15m` | Lifetime of an access token (`expires_in` in seconds), at most `24h`: a longer value stops the gateway at start-up; a non-positive value falls back to the default. Clients get a new one with their refresh token (see [Authentication and authorization](#authentication-and-authorization)) |
...
| `OWNER_NEGATIVE_CACHE_TTL` | `5s` | How long the gateway remembers that the account service doesn't know an account; `0` turns it off and a negative value stops the gateway at start-up |
...
| `POST` | `/identities/verify` | `{email, password}` | `200` `{user_id}`, `401 INVALID_CREDENTIALS`, `429 TOO_MANY_ATTEMPTS` (with `Retry-After`), `503 SERVICE_BUSY` |
| `POST` | `/sessions` | `{user_id}` | `201` `{refresh_token, expires_at}` (a new session), `422 VALIDATION_ERROR` (`user_id`) |
| `POST` | `/sessions/rotate` | `{refresh_token}` | `200` `{user_id, refresh_token, expires_at}`, `401 INVALID_SESSION` |
| `POST` | `/sessions/revoke` | `{refresh_token}` | `204`, also for an unknown, expired or already revoked token |
...
bcrypt is deliberately slow, so the service runs at most `IDENTITY_HASH_CONCURRENCY` hashes and comparisons at once (default twice `GOMAXPROCS`; values below `1` fall back to the default), the dummy comparison included. A request that can't start its hash within 2 s, or whose caller gives up first, answers `503 SERVICE_BUSY` without touching the stored identities, so a burst of logins queues briefly and then sheds load instead of starving the rest of the service. Every `503 SERVICE_BUSY` answers `Retry-After: 1`.

**Sessions.** A refresh token is 32 random bytes, base64url-encoded, and only its SHA-256 hash is stored, in `refresh_tokens` (migrations 009 and 012) with a TTL of `REFRESH_TOKEN_TTL` (default `720h`). Every login or registration starts a new family; `POST /sessions/rotate` marks the token it gets `rotated` with a lightweight transaction and issues the next token of the same family, which expires `REFRESH_TOKEN_TTL` later but never after the family is `REFRESH_FAMILY_MAX_AGE` old (default `2160h`), so a session has to log in again at least every 90 days. A refresh token works once: presenting a `rotated` or `revoked` token, or losing the race to rotate the same token, revokes the whole family and answers `401 INVALID_SESSION`. A stolen token is not refused on its own: whoever presents it first gets a fresh token and can keep rotating, and the family is revoked as soon as either party presents a token that was already used — typically the owner on their next refresh. If the owner never refreshes again, the thief keeps the session until the family reaches `REFRESH_FAMILY_MAX_AGE`. An unknown, malformed or expired token answers the same `401`. `POST /sessions/revoke` revokes the family of the token it gets. Revoking writes a marker in `revoked_families` (migration 011) before it marks every token of the family (`sessions_by_family`, migration 010) as `revoked`, and a rotation checks the marker before and after it rotates, so a revocation wins over a rotation running at the same time. A value of `REFRESH_TOKEN_TTL` or `REFRESH_FAMILY_MAX_AGE` that isn't positive falls back to the default.

**Login lockout.** `POST /identities/verify` counts attempts per normalised e-mail in `login_failures` (migration 013), for unknown e-mails exactly like registered ones, so a lockout doesn't reveal whether an e-mail exists. Each attempt is reserved with a lightweight transaction before the password is checked, so at most `LOGIN_MAX_FAILURES` (default `5`) passwords are compared per e-mail per window however many requests arrive at once. Once `LOGIN_MAX_FAILURES` attempts count within `LOGIN_LOCKOUT_WINDOW` (every reserved attempt counts, including those still being checked, until a success clears the count) (default `15m`, a fixed window that starts with the first failure and is measured by the service's clock), every verification of that e-mail answers `429 TOO_MANY_ATTEMPTS` with `Retry-After` set to the seconds left in the window, even with the right password. A successful login clears the count; an attempt that fails for any reason other than wrong credentials — `503 SERVICE_BUSY`, or an identity lookup error that answers `500` — gives its reservation back. An attempt that keeps losing the race to concurrent attempts on the same e-mail, or whose reservation write has an unknown outcome, answers `429 TOO_MANY_ATTEMPTS` with `Retry-After: 1`; when the count can't be read or written, verification answers `503 SERVICE_BUSY` without comparing the password. An e-mail that registration would refuse as malformed is not counted and gets no row: it answers `401 INVALID_CREDENTIALS` after the same dummy comparison as an unknown e-mail. `LOGIN_MAX_FAILURES` below `1` and a non-positive `LOGIN_LOCKOUT_WINDOW` fall back to the defaults.

**Known limitations.** Anyone who knows an e-mail can lock it for `LOGIN_LOCKOUT_WINDOW` with `LOGIN_MAX_FAILURES` wrong passwords (five requests by default), a trade-off accepted to cap guessing across addresses. `POST /sessions` trusts the `user_id` it is given, so whoever reaches the service's port can start a session for any user: only the gateway may reach it, and the service's compose file publishes it on `127.0.0.1:8082` only. A rotation whose Cassandra write times out may have been applied although it answered an error: a client that retries with the same refresh token then revokes its own family and has to log in again.
...
Every `/api/v1` route except `POST /api/v1/auth/register`, `/auth/login`, `/auth/refresh`, `/auth/logout` and `GET /api/v1/openapi.yaml` requires an access token in `Authorization: Bearer <token>`. Users register and log in with an e-mail and a password: the gateway checks them through the account service's internal identity endpoints (see [Account Service](#account-service)), signs the access token itself and starts a refresh session in the account service.

| Method | Path | Body | Answer |
|---|---|---|---|
| `POST` | `/api/v1/auth/register` | `{email, password}` | `201` `{user_id, access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `409 EMAIL_TAKEN`, `422 VALIDATION_ERROR`, `503 SERVICE_BUSY` |
| `POST` | `/api/v1/auth/login` | `{email, password}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_CREDENTIALS` (with `WWW-Authenticate: Bearer`), `422 VALIDATION_ERROR`, `429 TOO_MANY_ATTEMPTS` (with `Retry-After`), `503 SERVICE_BUSY` |
| `POST` | `/api/v1/auth/refresh` | `{refresh_token}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_SESSION` (with `WWW-Authenticate: Bearer`; also for a token longer than 256 bytes, refused without calling the account service), `422 VALIDATION_ERROR`, `503 SERVICE_BUSY` |
| `POST` | `/api/v1/auth/logout` | `{refresh_token}` | `204`, also for an unknown, expired or already revoked token and for one longer than 256 bytes, which never reaches the account service; `422 VALIDATION_ERROR`, `503 SERVICE_BUSY` |

The e-mail is trimmed and lower-cased, so `Ana@Example.com` and `ana@example.com` are the same user. Registration takes a valid e-mail of at most 254 characters (`details` `email`: `required`, `email` or `max`) and a password of 8 to 72 bytes (`password`: `required` or `length`), and answers with a new random user id. Login only requires both fields (`required`) and gives the same `401 INVALID_CREDENTIALS` for an unknown e-mail, a wrong password, an e-mail that could never have been registered and a password longer than 72 bytes, so the answer doesn't tell which one it was. Registration, on the other hand, does reveal whether an e-mail is already registered: a second registration answers `409 EMAIL_TAKEN`, which is why both routes sit behind the stricter rate limit below. `/auth/refresh` and `/auth/logout` answer `422 VALIDATION_ERROR` (`details` `refresh_token`: `required`) without a refresh token. All four answer `413 PAYLOAD_TOO_LARGE` for a body over 16 KiB, registration and login answer `503 SERVICE_BUSY` with `Retry-After: 1` when the account service is already hashing as many passwords as it allows (and login also when the account service can't read or write its login-failure counters), and all four answer `502 UPSTREAM_UNAVAILABLE` when the account service can't be reached or answers anything unexpected, a redirect included (the gateway never follows one).

**Tokens.** The access token is a JWT signed with HMAC-SHA256 (HS256) and `JWT_SECRET`, carrying `sub` (the user id), `iss` (`fintech-gateway`), `iat`, `exp` and `jti` (a random id). It lives for `JWT_TTL` (default `15m`, at most `24h`); `expires_in` is that lifetime in seconds. The gateway accepts only HS256 tokens it issued itself: any other `alg` (`none` included), another issuer, a bad signature, a missing or past `exp` (with 30 s of leeway for clock skew) or a `sub` that isn't a user id is refused. There is no revocation list for access tokens: logging out ends the session, but an access token already issued stays valid until it expires, and so does one that leaks. A missing, malformed or refused token answers `401 UNAUTHORIZED` with `WWW-Authenticate: Bearer`. The gateway removes the `Authorization` header before it proxies a read, so the domain services never see the token.

**Sessions.** Registration and login also return a refresh token, an opaque string the account service stores only as a SHA-256 hash, and `refresh_expires_in`, its lifetime in seconds (`REFRESH_TOKEN_TTL`, default `720h`). `POST /auth/refresh` exchanges it for a new access token and a new refresh token; the one sent stops working. Every refresh token issued from one login or registration belongs to the same session: sending a refresh token that was already used revokes the whole session, so the newest token stops working too and the client has to log in again — whoever sends the used token, the owner or someone who stole a token, and a session can't be refreshed past `REFRESH_FAMILY_MAX_AGE` (default `2160h`) after its login. `POST /auth/logout` revokes the session of the refresh token it gets and answers `204` whether or not the token was still valid, so it can be retried safely; it answers `502 UPSTREAM_UNAVAILABLE` when the account service can't be reached or fails, and then the outcome is unknown: the session may or may not have been revoked, so retry the logout, which is idempotent. A refresh that answers a `5xx` may still have rotated the token: retrying it with the same refresh token then revokes the session, and the client logs in again. The details are in [Account Service](#account-service).

**Login lockout.** Once `LOGIN_MAX_FAILURES` (default `5`) login attempts for one e-mail count within `LOGIN_LOCKOUT_WINDOW` (default `15m`; every attempt counts until a successful login clears the count), every login for that e-mail answers `429 TOO_MANY_ATTEMPTS` with `Retry-After` (the seconds left in the window) until the window ends, even with the right password. It counts per e-mail across every client address, applies to unknown e-mails exactly like registered ones, so a `429` doesn't reveal which e-mails exist, and a successful login resets it. A malformed e-mail is not counted: it answers `401 INVALID_CREDENTIALS` like an unknown one. The gateway passes the account service's `Retry-After` through on `429` and `503` and lists it in `CORS_EXPOSED_HEADERS` by default, so browser clients can read it. Anyone can lock someone else's e-mail for 15 minutes with five wrong passwords; that is the price of a limit that attackers can't spread across addresses.

**Rate limit.** On top of the general limit, `/auth/register`, `/auth/login`, `/auth/refresh` and `/auth/logout` share a stricter one: `AUTH_RATE_LIMIT_REQUESTS` (default `10`) per `AUTH_RATE_LIMIT_WINDOW` (default `1m`), counted per client address exactly like the general one (an IPv6 client by its /64 network) and answering `429 RATE_LIMIT_EXCEEDED`. It slows down password guessing; raise it only for test runs (see [End-to-end and load tests](#end-to-end-and-load-tests)). The limit is per client address; guesses spread over many addresses are capped per e-mail by the login lockout above.
...
Anything else answers `403 FORBIDDEN`. An account's owner comes from the account service's internal `GET /accounts/{id}/owner` and is kept in the gateway's memory for `OWNER_CACHE_TTL` (default `1m`, at most 10,000 accounts); an account's owner never changes, so the cache can't hand an account to someone else. Concurrent lookups of the same account share one call to the account service. An account the account service doesn't know answers `404 ACCOUNT_NOT_FOUND` from the gateway, whether it is named in the path or in a command body, and that answer is remembered for `OWNER_NEGATIVE_CACHE_TTL` (default `5s`, `0` turns it off, at most 1,000 unknown accounts, kept apart from the owners so a flood of random ids never pushes a known owner out), so a burst of requests for an unknown id costs one lookup, while an account created a moment ago is found at most that long after it exists; a failed lookup answers `502 UPSTREAM_UNAVAILABLE` and isn't cached. Command bodies are validated first, so an invalid command still answers `422` before its account is looked up. An account id in the path of `GET /accounts/{id}`, `GET /accounts/{account_id}/transactions` or `GET /accounts/{account_id}/payments` that is not a UUID answers `422 VALIDATION_ERROR` (`details` `{"id":"uuid"}` or `{"account_id":"uuid"}`) from the gateway itself, before any lookup; a percent-encoded id is decoded for the ownership check and then refused the same way, so it never reaches a service. `GET /transactions/{id}` and `GET /payments/{id}` fetch the record and hand it over only to its owner, so an unknown id still answers `404 TRANSACTION_NOT_FOUND` or `404 PAYMENT_NOT_FOUND`, while another user's record answers `403`. The recipient of a transfer can read it by id, but the gateway removes `description` and `idempotency_key` first, since those were chosen by the sender.

The services behind the gateway trust it: there is no service-to-service authentication, so their own ports (`8082`–`8085`), with the account service's internal identity, session and owner endpoints, must not be reachable from outside. The compose files publish them only for development, the account service's on `127.0.0.1` only, since its `POST /sessions` starts a session for whatever user id it is given; the payment service's webhook, which checks its own signature, is the one route a provider needs to reach.

```bash
curl -s -X POST localhost:8081/api/v1/auth/register \
  -H 'Content-Type: application/json' \
  -d '{"email":"ana@example.com","password":"correct-horse-battery"}'
# {"success":true,"data":{"user_id":"5d1e7c2a-0a6b-4c1e-9f4e-2b6f7a8c9d01","access_token":"eyJhbGciOiJIUzI1NiIs…","token_type":"Bearer","expires_in":900,"refresh_token":"qdJaufDW68Z4…","refresh_expires_in":2591999}}

SESSION=$(curl -s -X POST localhost:8081/api/v1/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"email":"ana@example.com","password":"correct-horse-battery"}')
TOKEN=$(echo "$SESSION" | jq -r .data.access_token)
REFRESH=$(echo "$SESSION" | jq -r .data.refresh_token)
...
curl -s -X POST localhost:8081/api/v1/auth/refresh \
  -H 'Content-Type: application/json' -d "{\"refresh_token\":\"$REFRESH\"}"
# {"success":true,"data":{"access_token":"eyJhbGciOiJIUzI1NiIs…","token_type":"Bearer","expires_in":900,"refresh_token":"Vb3n0cW1x7Qe…","refresh_expires_in":2591999}}

curl -s -X POST localhost:8081/api/v1/auth/refresh \
  -H 'Content-Type: application/json' -d "{\"refresh_token\":\"$REFRESH\"}"
# {"success":false,"error":{"code":"INVALID_SESSION","message":"refresh token is invalid or expired"}}
```

The second refresh reuses a token that was already rotated, so it also revoked the session: the refresh token returned by the first one no longer works either. `POST /api/v1/auth/logout` with `{"refresh_token": …}` ends a session on purpose and answers `204`.
...
**Upgrading from a release before Sprint 12.** `JWT_TTL` now defaults to `15m` instead of `1h`; set it explicitly to keep the old lifetime. Registration and login now also return `refresh_token` and `refresh_expires_in`, which clients that ignore them can keep ignoring, logging in again when the access token expires. Access tokens issued before the upgrade stay valid until they expire. The account service applies migrations 009 to 013 (`refresh_tokens`, `sessions_by_family`, `revoked_families`, `refresh_tokens.family_created_at`, `login_failures`) at start-up; deploy it before or together with the gateway, because registration and login call its new session endpoint and answer `502 UPSTREAM_UNAVAILABLE` until it exists. Five failed logins for an e-mail within 15 minutes now lock it for the rest of the window; anything that logs in repeatedly with a wrong password on purpose, such as a test, needs a fresh e-mail for each case.
...
│   │   │   ├── app/handlers/              command endpoints, register/login/refresh/logout, ownership guard, read proxy, OpenAPI handler
│   │   │   └── infrastructure/
│   │   │       ├── auth/                  JWT issuer and verifier, bearer middleware
│   │   │       ├── identity/              account service identity and session client
│   │   │       ├── owners/                account owner lookup with an in-memory cache and collapsed concurrent lookups
...
│   │   │   ├── app/
│   │   │   │   ├── models/                domain types
│   │   │   │   ├── services/              account, customer, identity and session use cases
│   │   │   │   └── handlers/              command dispatcher, DLQ, read, identity and session endpoints
│   │   │   └── infrastructure/
...
- [x] **Sprint 12** — sessions: rotating refresh tokens with logout, per-e-mail login lockout and owner lookup hardening

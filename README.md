...
| `POST` | `/api/v1/auth/refresh` | `{refresh_token}` | `200` `{access_token, token_type: "Bearer", expires_in, refresh_token, refresh_expires_in}`, `401 INVALID_SESSION` (with `WWW-Authenticate: Bearer`), `422 VALIDATION_ERROR` |
| `POST` | `/api/v1/auth/logout` | `{refresh_token}` | `204`, also for an unknown, expired or already revoked token; `422 VALIDATION_ERROR` |
...
**Login lockout.** After `LOGIN_MAX_FAILURES` (default `5`) failed logins for one e-mail within `LOGIN_LOCKOUT_WINDOW` (default `15m`), every login for that e-mail answers `429 TOO_MANY_ATTEMPTS` with `Retry-After` (the seconds left in the window) until the window ends, even with the right password. It counts per e-mail across every client address, applies to unknown e-mails exactly like registered ones, so a `429` doesn't reveal which e-mails exist, and a successful login resets it. The gateway passes the account service's `Retry-After` through on `429` and `503`. Anyone can lock someone else's e-mail for 15 minutes with five wrong passwords; that is the price of a limit that attackers can't spread across addresses.
...
- [x] **Sprint 12** — sessions: rotating refresh tokens with logout, per-account login lockout and owner lookup hardening

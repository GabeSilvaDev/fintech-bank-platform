...
func reply(kind string, tx *models.Transaction, step models.Step, balance int64, reason string) services.Reply {
	return services.Reply{Kind: kind, Reference: tx.ID.String(), IdempotencyKey: models.StepKey(tx.ID, step), BalanceAfter: domain.AmountFromCents(balance), Reason: reason, TraceID: "trace-9"}
}
...
func TestDepositCreditedCompletes(t *testing.T) {
	...
	res, err := h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountCredited, tx, models.StepCredit, 13000, ""))
	...
	assert.Equal(t, domain.AmountFromCents(3000), payload.Amount)
	assert.Equal(t, domain.AmountFromCents(13000), payload.BalanceAfter)
...
func TestWithdrawalDebitedCompletesAndRejectionsFail(t *testing.T) {
	...
	res, err := h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountDebited, tx, models.StepDebit, 7000, ""))
	assert.NoError(t, err)
	...
	assert.Equal(t, domain.AmountFromCents(7000), res.Messages[0].Event.Payload.(events.TransactionCompletedPayload).BalanceAfter)
...
func TestTransferHappyPath(t *testing.T) {
	...
	assert.Equal(t, domain.AmountFromCents(3000), credit.Amount)
	...
	res, err = h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountCredited, tx, models.StepCredit, 3000, ""))
	assert.NoError(t, err)
	stored := h.repo.Transactions[tx.ID]
	assert.Equal(t, models.StatusCompleted, stored.Status)
...
func TestTransferCreditRejectedReversesThenReversed(t *testing.T) {
	...
	res, err = h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountCredited, tx, models.StepReversal, 10000, ""))
	assert.NoError(t, err)
...
func TestApplyIgnoresUnrelatedOrStaleReplies(t *testing.T) {
	...
	neverReachesRepo := []services.Reply{
		...
		reply(events.EventTypes.AccountDebited, tx, models.StepCredit, 100, ""),
		reply(events.EventTypes.AccountCreated, tx, models.StepDebit, 100, ""),
	}
	...
	staleButAttempted := []services.Reply{
		reply(events.EventTypes.AccountCredited, tx, models.StepCredit, 100, ""),
		reply(events.EventTypes.AccountCredited, tx, models.StepReversal, 100, ""),
	}
...
func TestOnCreditedIgnoresNonDepositNonTransferTypes(t *testing.T) {
	...
	res, err := h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountCredited, tx, models.StepCredit, 7000, ""))
...
func TestTransferCompletedWithoutStoredFromBalance(t *testing.T) {
	...
	assert.NoError(t, err)
	completed := res.Messages[0].Event.Payload.(events.TransferCompletedPayload)
	assert.Equal(t, domain.AmountFromCents(0), completed.FromBalanceAfter)
}
...
func TestApplyPropagatesRepositoryErrors(t *testing.T) {
	...
	_, err := h.service.ApplyAccountEvent(context.Background(), reply(events.EventTypes.AccountCredited, tx, models.StepCredit, 100, ""))

...
func TestReplyDispatcherRoutesAccountEvents(t *testing.T) {
	...
	debited := events.NewAccountEvent(events.EventTypes.AccountDebited, events.AccountDebitedPayload{AccountID: payment.AccountID.String(), Amount: 42.5, BalanceAfter: 57.5, Reference: models.Reference(payment.ID), IdempotencyKey: models.StepKey(payment.ID, models.StepDebit)}).WithTraceID("trace-7")
	...
	rejected := events.NewAccountEvent(events.EventTypes.DebitRejected, events.DebitRejectedPayload{AccountID: other.AccountID.String(), Amount: 42.5, Reason: "insufficient_funds", Reference: models.Reference(other.ID), IdempotencyKey: models.StepKey(other.ID, models.StepDebit)})

...
func TestConsumerAppliesCommandsEndToEnd(t *testing.T) {
	...
	results := kafka.NewReader(kafka.ReaderConfig{Brokers: addrs, GroupID: "it-results-" + uuid.NewString(), Topic: events.Topics.AccountEvents, StartOffset: kafka.LastOffset, MinBytes: 1, MaxBytes: 1 << 20})
	defer results.Close()
